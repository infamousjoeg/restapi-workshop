...
import {
  default as cyberark
} from "./src/themes/cyberark";

import "prismjs/components/prism-powershell";

<!-- Default theme -->
export const theme = cyberark;
...
<Head>
  ...
  <link href="./src/fonts/stylesheet.css" rel="stylesheet" />
  <link href="./src/themes/cyberark.css" rel="stylesheet" />
...
<ul>
  <Appear>
    <li>Create Safe & Safe Members Script<br />
      <i>(CyberArk Safe Factory)</i></li>
    <li>Q&A Session</li>
  </Appear>
</ul>
...
<div style="padding-left: 50px; padding-right: 50px;">
  <ul>
    <Appear>
      ...
      <li>No... I don't know when we can Move Accounts via REST API 🙁</li>
      <li>I also don't know when we'll be able to automate linked accounts like Logon & Reconcile 😫</li>
    </Appear>
  </ul>
</div>
...
<ul>
  <Appear>
    <li>Contains CyberArk Supported Examples</li>
    <li>Account Onboarding Utility</li>
    <li>Automatic Onboarding Rules</li>
    <li>PSM, Platforms, Security Events, and more!</li>
  </Appear>
</ul>
...
```diff 4:5 title="Create Command" subtitle="This is the parameter to set to create"
...
<ul>
  <Appear>
    <li>Accessed at <u>/PasswordVault/swagger</u></li>
    <li>Similar to Postman Online Docs & Collection</li>
...
Docs available online at [https://cybr.rocks/RESTAPI](https://cybr.rocks/RESTAPI)

Collection available for use in Postman app
...
```powershell title="How to update psPAS" subtitle="Updating is as easy as installing"
Update-Module psPAS
```

```powershell title="How to use psPAS" subtitle="Start your script with this to use psPAS in it"
Import-Module psPAS
```
...
## Example One-Liners & Scripts

This will be covered later during our _psPAS Deep Dive_.
...
<ul>
  <Appear>
    <li>Get-CCPCredential</li>
    <li>Get-AIMCredential</li>
  </Appear>
</ul>
...
```powershell title="CredentialRetriever with CCP" subtitle="Simple function with typical metadata"
Get-CCPCredential -AppID DemoApp -Safe DemoSafe -UserName DemoUser -URL https://components.cyberarkdemo.com
```
...
```powershell 1[49:97] title="CredentialRetriever with CCP" subtitle="You can also use ObjectName like usual"
Get-CCPCredential -AppID DemoApp -Safe DemoSafe -ObjectName PlatformId-DemoSafe-Address-DemoUser -URL https://components.cyberarkdemo.com
```
...
```powershell title="CredentialRetriever with CCP" subtitle="Script Example"
Import-Module CredentialRetriever

do {
    $response = Get-CCPCredential -AppID DemoApp -Safe DemoSafe -UserName DemoUser -URL https://components.cyberarkdemo.com
...
```diff 1 title="CredentialRetriever with CCP" subtitle="First, we need to import the module into our script"

```

```diff 3:5 title="CredentialRetriever with CCP" subtitle="Here we use a do...until loop to ensure the password is not changing"

```

```diff 4 title="CredentialRetriever with CCP" subtitle="Our response from CCP will be stored in the $response var"

```

```diff 5[11:55] title="CredentialRetriever with CCP" subtitle="If the password is not changing, we can continue..."

```

```diff 7:9 title="CredentialRetriever with CCP" subtitle="...and the rest of the script will complete."
...
```powershell title="CredentialRetriever with Credential Provider" subtitle="Simple function with minimal config"
Set-AIMConfiguration -ClientPath "C:\Program Files (x86)\CyberArk\ApplicationPasswordSdk\CLIPasswordSDK.exe"

Get-AIMCredential -AppID DemoApp -Safe DemoSafe -UserName DemoUser -RequiredProps Username,Address
```

```diff 1 title="CredentialRetriever with Credential Provider" subtitle="First, the path to CLIPasswordSDK.exe should be set"

```

```diff 3 title="CredentialRetriever with Credential Provider" subtitle="Then, the credential can be retrieved using the function"

```

```diff 3[68:99] title="CredentialRetriever with Credential Provider" subtitle="RequiredProps will only return the properties listed here"

```

```diff 3[83:99] title="CredentialRetriever with Credential Provider" subtitle="Here, we're just asking for Username, Address, and Content (which is default)"
...
```powershell 2 title="MFA Support" subtitle="Append Mode"
New-PASSession -BaseURI https://components.cyberarkdemo.com -Type radius -Credentials $(Get-Credentials) `
    -OTPMode Append -OTP 123456
```

```powershell 2 title="MFA Support" subtitle="Challenge/Response Mode"
New-PASSession -BaseURI https://components.cyberarkdemo.com -Type radius -Credentials $(Get-Credentials) `
...
```diff 2 title="Piping Support for Advanced Users" subtitle="... if CyberArk_Vault_Admins is not a member of the safe..."

```

```diff 3 title="Piping Support for Advanced Users" subtitle="Write the safe name to output for our audit report"
